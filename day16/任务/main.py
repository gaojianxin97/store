...
def addUser():
    ...
    start = Bank.bank_addUser(account,User.getUsername(),User.getPassword(),Address.getCounrry(),
                              Address.getProvicne(),Address.getStreet(),Address.getDoor())
    if start == 1:
        print("开户成功!")
...
def seek():
    ...
    if start == 0:
        # 查询表中数据
        sql = "select * from bank where username = %s and password = %s"
